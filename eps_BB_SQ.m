...
% This is the modified version by SQ on 21-Nov-2016 to get n and k

function w = cef(z,N);

%Visualization Parameters (USER INPUT)
%lmin = 1e-7; %Minimum wavelength in visualization
%lmax = 1.25e-5; %Maximum wavelength in visualization
lmin = 2.5e-7; %Minimum wavelength in visualization
lmax = 2e-6; %Maximum wavelength in visualization
dl = 0.5e-8; %Visualization wavelength incrament
...
for lambda = lmin:dl:lmax;
    ...
    eps = epsf + sum(epsb);
    Reps_op(istep) = abs(real(eps));
    Ieps_op(istep) = abs(imag(eps));
    
    tp1 = real(eps);
	tp2 = imag(eps);
	tp3 = (1/sqrt(2)) * sqrt( tp1 + sqrt(tp1^2+tp2^2) );
	tp4 = sign(1)*(1/sqrt(2)) * sqrt( -tp1 + sqrt(tp1^2+tp2^2) );
    Refractive_index_op(istep) = tp3;
    Extinction_coefficient_op(istep) = tp4;
    ...
    %Visualzation over optical wavelengths: Refractive_index; Extinction_coefficient
    figure(2)
    plot(lambda,Refractive_index_op(istep),'o',lambda,Extinction_coefficient_op(istep),'+')
    hold on
    axis([lmin lmax 0 80])
    xlabel('Wavelength [m]')
    ylabel('Refractive index & Extinction coefficient')
    title('Relative Permittivity \epsilon_{r} = (n + j k)^2')
end

%opA = [lambda_op;Reps_op;Ieps_op];
opA = [lambda_op;Refractive_index_op;Extinction_coefficient_op];
fileID = fopen('RefractiveIndexINFORakic1998.csv','w');
